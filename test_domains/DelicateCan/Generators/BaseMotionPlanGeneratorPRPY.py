...
class BaseMotionPlanGeneratorPRPY(Generator):
    def __init__(self, ll_state=None, known_argument_values=None):
        ...
        with open(Config.MISC_DIR+"RobotModels/fetch/URDF/sbpl_global_params.yaml", 'r') as stream:
            try:
                extra_params = yaml.safe_load(stream)
            except yaml.YAMLError as exc:
                print(exc)
            stream.close()
        self.planner.SetPlannerParameters(extra_params)
    ...
    def get_traj_from_planner(self,base_goal_val):
        try:
            trajectory_object = self.planner.PlanToBasePose(self.robot,base_goal_val)
        except PlanningError:
            print(__file__.split('/')[-1],": Could not find base motion plan")
            return None
        status = planningutils.SmoothAffineTrajectory(trajectory_object, [4.,4.,4.], [.5, .5, .5], plannername='LinearTrajectoryRetimer')
